...
const { MealService } = require('../lib/meals/meal-service');
const { OfficeService } = require('../lib/offices/office-service');
const { PaymentService } = require('../lib/payments/payment-service');
const { ReportService } = require('../lib/reports/report-service');
const { getOfficeTomorrowDate, getOfficeCurrentDate } = require('../lib/utils/dates');
...
async function run50UserSimulation() {
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    const pref = i <= 25 ? 'flexible' : i <= 40 ? 'always-veg' : 'always-non-veg';
  ...
  const { defaultedCount, affectedUsers } = await MealService.autoDefaultMealsForOffice(testOfficeId, targetDate);
  ...
  const catererSummary = await ReportService.generateCatererSummary(testOfficeId, targetDate);
  assert(catererSummary.vegCount === 30, 'Caterer summary contains 30 Veg');
  assert(catererSummary.nonVegCount === 15, 'Caterer summary contains 15 Non-Veg');
  assert(catererSummary.whatsAppText.includes('TechCorp Labs'), 'WhatsApp message includes office name');
  assert(catererSummary.whatsAppText.includes('Total Meals: 45'), 'WhatsApp message includes total meal count');
  console.log('Generated WhatsApp Message:\n' + catererSummary.whatsAppText);
  ...
  assert(user1Payment && user1Payment.status === 'pending', 'User 1 bill is initially pending');
